...
#'     `recyclable_n`   \tab Evaluates whether the collective set of positive whole-number values in `...` arguments represent recyclable arguments.
#'   \cr                \tab  
#'   \cr `recyclable`   \tab Evaluates whether `...` arguments are recyclable.
#'   \cr                \tab  
#'   \cr    `recycle`   \tab Recycles `...` arguments in the environment of the calling function.
#' }
...
recyclable_n <- function(..., n. = NULL, min. = NULL, max. = NULL, targ. = NULL) {
  ...
  errs <- c(f0(ok.d , NULL, .errx("[...] is empty.")),
            f0(ok.n , NULL, .errx("[...] must atomize (?av) to a complete positive whole-number vec (?cmp_psw_vec).")),
            f0(ok.n., NULL, .errx("[n.] must be NULL or a complete positive whole-number vec (?cmp_psw_vec).")),
            f0(ok.m., NULL, .errx("[min.] must be NULL or a complete positive whole-number scalar (?cmp_psw_scl).")),
            f0(ok.x., NULL, .errx("[max.] must be NULL or a complete positive whole-number scalar (?cmp_psw_scl).")),
            f0(ok.t., NULL, .errx("[targ.] must be NULL or a complete positive whole-number scalar (?cmp_psw_scl).")))
...
recyclable <- function(..., n. = NULL, min. = 1, max. = NULL, targ. = NULL, err. = FALSE) {
  ...
  errs <- c(f0(ok.0, NULL, .errx("[...] is empty.")),
            f0(ok.v, NULL, .errx("Arguments in [...] must be atomic vecs (?atm_vec).")),
            f0(ok.e, NULL, .errx("[err.] must be scalar TRUE or scalar FALSE.")))
  if (!is.null(errs)) {stop(errs)}
  out <- recyclable_n(lengths(dots), n. = n., min. = min., max. = max., targ. = targ.)
  if (err. & !out) {stop(.errx("[...] arguments are not recyclable."))}
  out
}
