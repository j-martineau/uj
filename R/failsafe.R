...
#' @details Attempts to call \code{identity(x)}. If an error is produced in
#'   doing so, returns an object of class \code{'error'} or of class
...
#' @describeIn failsafe Evaluates whether calling \code{identity(x)} produced an
#'   error.
#' @export
isERR <- function(x) {any(class(failsafe(x)) %in% c("error", "simpleError"))}

#' @describeIn failsafe Evaluates whether calling \code{identity(x)} did not
#'   produce an error.
#' @export
notERR <- function(x) {!any(class(failsafe(x)) %in% c("error", "simpleError"))}

#' @describeIn failsafe Gets any error message associated with calling
