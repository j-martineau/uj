...
rng2lev <- function(x, cuts, levs) {
  errs <- c(f0(pop_srt(x)       , NULL, .errx("[x] must be an atomic sortable object (?isrt).")),
            f0(cmp_srt_vec(cuts), NULL, .errx("[cuts] must be a complete sortable vec (?cmp_srt_vec).")),
            f0(cmp_srt_vec(levs), NULL, .errx("[levs] must be a complete sortable vec (?cmp_srt_vec).")))
  if (!is.null(errs)) {stop(errs)}
  errs <- c(f0(all(sort(cuts) == cuts)             , NULL, .errx("[cuts] must be sorted in increasing order.")),
            f0(length(unique(cuts)) == length(cuts), NULL, .errx("[cuts] contains duplicate values.")),
            f0(length(cuts) == length(levs) - 1    , NULL, .errx("length(cuts) must equal length(levs) - 1.")),
            f0(comparable(x, cuts, recycle. = F)   , NULL, .errx("[x] and [cuts] must be of comparable modes (?comparable).")))
  if (!is.null(errs)) {stop(errs)}
  browser()
  y <- x
  y[1:length(y)] <- NA
