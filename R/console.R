...
#' @export
console_uj <- function() {help("console_uj", package = "uj")}

#' @describeIn console_uj Collapse \code{...} into a character scalar and print
#'   it to the console.
#' @export
say <- function(...) {cat(dw0(...))}

#' @describeIn console_uj Collapse \code{...} into a character scalar, prefix
#'   and post-fix a newline, and print the result to the console.
#' @export
cat0 <- function(...) {say("\n", ..., "\n")}

#' @describeIn console_uj Print \code{'Hit [enter] or [return] to continue'}
