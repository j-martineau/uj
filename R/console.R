...
#' @description \itemize{
#'   \item **`say`**: collapses the message in `...` to a character scalar and either (a) prints it to the console or (b) structures it as a certain level of update and then prints it to the console. See details.
#'   \item **`cat0`**: collapses `...` to a character scalar message prefixed and suffixed with newlines and prints it to the console (ensures the message occurs on its own line of the console).
#'   \item **`continue`**: prints `"Hit [enter] or [return] to continue"` to the console and pause execution until the user hits the enter or return key.
#' }
...
#' \tabular{lll}{
#'   MESSAGE         \tab VALUE OF     \tab UPDATE
#'   \cr SUPPLIED    \tab THE `lev`    \tab PRINTED TO
#'   \cr IN `...`?   \tab ARGUMENT     \tab CONSOLE
#'   \cr yes         \tab `NA`         \tab `'___'`
...
#'   \cr no          \tab `NA`         \tab `'.'`
#' }
#' Other combinations not represented in the table above will result in an error. To better understand this user update system, an example is provided. If the current contents of the console were this:
#' ```
#' > (( existing content ))
#' AN EXISTING MESSAGE
#' ```
...
#' ```
#' (( existing content ))_a_random_message..
#' L1
#' | L2
#' | L2B
#' | > L3
...
#' @return `NULL`. Called for side effects of console-based user updates or pausing execution for until user indicates execution should continue.
#' @export
say <- function(..., lev = NA) {
