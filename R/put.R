...
#' putc(egDtf, 1:5, 1)
#' @export
put_help <- function() {utils::help("put_help", package = "uj")}

#' @describeIn put_help Puts vector `v` into `e`-indexed elements of `x` (assumed to be a vector or \link[=VLS]{vlist}).
#' @export
put <- function(x, v, e) {x[e] <- v; x}

#' @describeIn put_help Puts vector `v` into `c`-indexed column of `x` (assumed to be a data.frame or matrix).
#' @export
putr <- function(x, v, r) {x[r, ] <- v; x}

#' @describeIn put_help Puts vector `v` into `r`-indexed row of `x` (assumed to be a data.frame or matrix).
#' @export
