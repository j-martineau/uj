...
callers <- function(gens = NULL) {
  if (!uj::inll(gens) & !uj::cmp_psw_vec(gens)) {stop(uj:::.errs("[gens] must be NULL or a positive whole-number vec (?cmp_psw_vec)."))}
  x <- base::rev(base::as.character(base::sys.calls()))                          # get the call stack
  ...
  if (base::max(gens) > base::length(x)) {stop(uj:::.errs("One or more elements of [gens] is greater than the number of calling functions."))}
  x[gens]                                                                        # otherwise, if the specified generations exist, return just those gens
}

#' @rdname callers
#' @export
caller <- function() {uj::callers(1)}
