...
#'                `name`           \tab Naming                                \cr   \tab  }
#' \tabular{ll}{  `rcnames, rcn`   \tab Row and column names                  \cr
#'                `rnames, rn`     \tab Row names                             \cr
#'                `cnames, cn`     \tab Column names                          \cr
#'                `enames, en`     \tab Element names                         \cr
#'                `dnames, dn`     \tab `...` arg names                       \cr   \tab  }
#' \tabular{ll}{  `rcnamed`        \tab Are both rows and columns named?      \cr
#'                `cnamed`         \tab Are columns of `x` named?             \cr
#'                `dnamed`         \tab Are `...` arguments named?            \cr
#'                `enamed`         \tab Are elements of `x` named?            \cr
#'                `rnamed`         \tab Are rows of `x` named?                \cr   \tab  }
#' \tabular{ll}{  `namerc`         \tab Name rows and columns of `x`          \cr
...
#' @return **A \link[=VEC]{vec}, \link[=VLS]{vlist}, matrix, or data.frame** \cr `name`
#' \cr\cr  **A vector or a vlist of length** `2`                             \cr `get_names`
#' \cr\cr  **A matrix or data.frame**                                        \cr `namerc, namer,namec`
#' \cr\cr  **A \link[=VLS]{vlist} of length** `2`                            \cr `rcnames, rcn`
#' \cr\cr  **A `1`-element \link[=VLS]{vlist}**                              \cr `namel`
#' \cr\cr  **A character vector**                                            \cr `rnames, cnames, enames`  \cr `rn, cn, en`
#' \cr\cr  **A logical scalar**                                              \cr `rcnamed, rnamed, cnamed` \cr `dnamed, enamed, named`
#' \cr\cr  **A vector or \link[=VLS]{vlist}**                                \cr `namev`
#' @examples
...
#' @export
namer <- function(x, rn) {
  uj::errs_if_nots(uj::MVC(x), "[x] must be an multivec (?MVC).", uj::cmp_vec(rn) & un::NR(x) == uj::N(rn), "[rn] must be a complete vec (?cmp_vec) of length [nrow(x)].", PKG = "uj")
  uj::RN(x) <- rn
  x
}
...
#' @export
namec <- function(x, cn) {
  uj::errs_if_nots(uj::MVC(x), "[x] must be an multivec (?MVC).", uj::cmp_vec(cn) & uj::NC(x) == uj::N(cn), "[cn] must be a complete vec (?cmp_vec) of length [ncol(x)].", PKG = "uj")
  uj::CN(x) <- cn
}
...
#' @rdname naming
#' @export
name <- function(x, en = NULL, rn = NULL, cn = NULL) {uj::f0(uj::D1D(x), uj::namee(x, en), uj::f0(uj::D2D(x), uj::namerc(x, rn, cn), uj::stopper("[x] must be a vec (?VEC), vlist (?VLS), matrix, or data.frame.", PKG = "uj")))}
...
named <- function(x, dim = 0, u = T, bl = F) {
  ...
  if (dim == 0) {                                                                # if inspecting for element names
    elabs <- uj::EN(x)                                                           # > get element names
...
get_names <- function(x, d = 0, u = T, err = F) {
  ...
  if (d == 0) {
    ...
    en
  } else if (!uj::D2D(x)) {uj::stopper("[x] must be a matrix or a data.frame when d is 1, 2, or 12.", PKG = "uj")} else if (d == 12) {
    rn <- uj::RN(x)
    cn <- uj::CN(x)
    if (err & (uj::N0(rn) | uj::N0(cn))) {uj::stopper("rows and/or columns of [x] are not named.", PKG = "uj")}
    else if (u & (uj::notUNQ(rn) | uj::notUNQ(cn))) {uj::stopper("row and/or column names of [x] are not unique.", PKG = "uj")}
    base::list(rows = rn, cols = cn)
  } else if (d == 1) {
    rn <- uj::RN(x)
    if (uj::N0(rn) & !err) {return(NULL)}
    else if (err & uj::N0(rn)) {uj::stopper("rows of [x] are not named.", PKG = "uj")}
    else if (u & notUNQ(rn)) {uj::stopper("row names of [x] are not unique.", PKG = "uj")}
    rn
  } else if (d == 2) {
    cn <- uj::CN(x)
    if (uj::N0(cn) & !err) {return(NULL)}
    else if (err & uj::N0(cn)) {uj::stopper("columns of [x] are not named.", PKG = "uj")}
    else if (u & notUNQ(cn)) {uj::stopper("column names of [x] are not unique.", PKG = "uj")}
    cn
  }
}
