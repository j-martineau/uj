...
#' @param .NA `TRUE` or `FALSE` indicating whether `NA` values qualify as `'logical'`.
...
check_lgl <- function(..., .NA = FALSE, .EXTRAS = NULL) {
  .NA <- uj::f0(uj:::.cmp_lgl_scl(.NA), .NA, F)
...
check_nas_or <- function(.FUNS, ..., .VALS = NULL) {
  ...
  for (i in 1:nX) {if (!uj::.NA(base::...elt(i))) {
    OK <- F
    for (Fun in .FUNS) {
      OK <- OK | base::eval(base::parse(text = base::paste0(Fun, "(base::...elt(i))")))
      if (OK & !base::is.null(.VALS)) {OK <- OK & base::all(base::...elt(i) %in% .VALS)}
...
check_cls <- function(.CLS, ...) {
  ...
  else {Message <- base::paste0("any class in c(", paste0(Message, collapse = ", "), ")")}
  for (i in 1:base::length(X)) {
    ClassesI <- base::class(X[[i]])
    if (!base::any(.CLS %in% ClassesI)) {uj::bankerr("[", Labels[i], "] must be of ", Message, ".", .GENS = 1, .D = "")}
  }
}
...
check_spec <- function(.SPEC, ..., .NA = F) {
  ...
  if (!OkNas) {Errors <- base::c(Errors, "[.NA] must be TRUE or FALSE.")}
