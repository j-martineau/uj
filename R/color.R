...
#' }
#' **Functions in this family** \tabular{rl}{
...
color <- function(x, lighten = 0, darken = 0, a = 1, comp = F) {
  ...
  reps <- base::max(ns) / ns
  errs <- base::c(uj::f0(uj::cmp_clr_vec(x)                  , NULL, "[x] must be a complete color vec (?cmp_clr_vec) [i.e., containing only valid character-mode color values]."),
                  uj::f0(uj::cmp_ppn_vec(lighten)            , NULL, "[lighten] must be a complete proportion vec (?cmp_ppn_vec). That is, in the interval [0, 1]."),
                  uj::f0(uj::cmp_ppn_vec(darken)             , NULL, "[darken] must be a complete proportion vec (?cmp_ppn_vec). That is, in the interval [0, 1]."),
                  uj::f0(uj::cmp_ppn_vec(a)                  , NULL, "[a] must be a complete proportion vec (?cmp_ppn_vec). That is, in the interval [0, 1]."),
                  uj::f0(uj::isTF(comp)                      , NULL, "[comp] must be TRUE or FALSE."),
                  uj::f0(base::all(reps == base::round(reps)), NULL, "[x], [lighten], [darken], and [a] are not recyclable (?recyclable)."))
  ...
  g[comp] <- 1 - g[comp]
  b[comp] <- 1 - b[comp]
  if (uj::idef(a)) {a <- a}                                                          # if an alpha value was supplied, use it
...
rgba <- function(r = 1, g = 1, b = 1, a = 1) {
  ...
  reps <- base::max(ns) / ns
  errs <- base::c(uj::f0(uj::cmp_ppn_vec(r)                  , NULL, "[r] must be a complete proportion vec (?cmp_ppn_vec). That is, in the interval [0, 1]."),
                  uj::f0(uj::cmp_ppn_vec(g)                  , NULL, "[g] must be a complete proportion vec (?cmp_ppn_vec). That is, in the interval [0, 1]."),
                  uj::f0(uj::cmp_ppn_vec(b)                  , NULL, "[b] must be a complete proportion vec (?cmp_ppn_vec). That is, in the interval [0, 1]."),
                  uj::f0(uj::cmp_ppn_vec(a)                  , NULL, "[a] must be a complete proportion vec (?cmp_ppn_vec). That is, in the interval [0, 1]."),
                  uj::f0(base::all(reps == base::round(reps)), NULL, "[r], [g], [b], and [a] are not recyclable (?recyclable)."))
  if (!base::is.null(errs)) {stop(uj::format_errs(pkg = "uj", errs))}
  grDevices::rgb(r, g, b, a)
}
...
hsva <- function(h = 1, s = 1, v = 1, a = 1) {
  ...
  reps <- base::max(ns) / ns
  errs <- base::c(uj::f0(uj::cmp_ppn_vec(h)                  , NULL, "[h] must be a complete proportion vec (?cmp_ppn_vec). That is, in the interval [0, 1]."),
                  uj::f0(uj::cmp_ppn_vec(s)                  , NULL, "[s] must be a complete proportion vec (?cmp_ppn_vec). That is, in the interval [0, 1]."),
                  uj::f0(uj::cmp_ppn_vec(v)                  , NULL, "[v] must be a complete proportion vec (?cmp_ppn_vec). That is, in the interval [0, 1]."),
                  uj::f0(uj::cmp_ppn_vec(a)                  , NULL, "[a] must be a complete proportion vec (?cmp_ppn_vec). That is, in the interval [0, 1]."),
                  uj::f0(base::all(reps == base::round(reps)), NULL, "[h], [s], [v], and [a] are not recyclable (?recyclable)."))
  if (!base::is.null(errs)) {stop(uj::format_errs(pkg = "uj", errs))}
...
blend <- function(x, y, wx = 1, wy = 1, a = NA) {
  ...
  reps <- base::max(ns) / ns
  errs <- base::c(uj::f0(uj::cmp_clr_vec(x)               , NULL, "[x] must be a complete color vec (?cmp_clr_vec)."),
                  uj::f0(uj::cmp_clr_vec(y)               , NULL, "[x] must be a complete color vec (?cmp_clr_vec)."),
                  uj::f0(uj::cmp_pos_vec(wx)              , NULL, "[wx] must be a complete positive numeric vec (?cmp_pos_vec)."),
                  uj::f0(uj::cmp_pos_vec(wy)              , NULL, "[wu] must be a complete positive numeric vec (?cmp_pos_vec)."),
                  uj::f0(base::all(reps == base::round(reps))     , NULL, "[a] must be [NA] or a complete proportion numeric vec(?cmp_ppn_vec)."),
                  uj::f0(uj::f0(uj::nas(a), T, uj::cmp_ppn_vec(a)), NULL, "[x], [y], [wx], [wy], and [a] are not recyclable (?recyclable)."))
  if (!base::is.null(errs)) {stop(uj::format_errs(pkg = "uj", errs))}
  ...
  a[!base::is.na(a)] <- a[!base::is.na(a)]                                                   # use any non-NA values in a to replace weighted alpha values
...
pal_cb <- function(ng, nc = NULL, a = 1) {
  errs <- base::c(uj::f0(uj::cmp_psw_scl(ng)                     , NULL, "[ng] must be a complete positive whole-number scalar (?cmp_psw_scl)."),
                  uj::f0(uj::f0(uj::inll(nc), T, uj::cmp_psw_scl), NULL, "[nc] must be NULL or a complete positive whole-number scalar (?cmp_psw_scl)."),
                  uj::f0(uj::cmp_ppn_scl(a)                      , NULL, "[a] must be a complete proportion scalar (?cmp_ppn_scl). That is, in the interval [0, 1]."))
  if (!base::is.null(errs)) {stop(uj::format_errs(pkg = "uj", errs))}
  out <- base::c("#4E79A7", "#F28E2B", "#E15759", "#76B7B2", "#59A14F", "#EDC948", "#B07AA1", "#FF9DA7", "#9C755F", "#BAB0AC", # bright colors
  ...
  if (nc > base::length(out)) {out <- base::rep.int(out, base::ceiling(nc / base::length(out)))} # if [nc] is larger than 20, repeat colors
...
pal_swatch <- function(x) {
  if (!uj::cmp_clr_vec(x)) {stop(uj::format_errs(pkg = "uj", "[x] must be a complete color vec (?cmp_clr_vec) [i.e., containing only valid character-mode color values]."))}
