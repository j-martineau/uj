...
go <- function(...) {
  ...
  if (base::length(file) != 2) {uj::stopperr("Invalid call.")}
  file <- file[2]
  if (base::is.null(file)) {
    path <- uj::choose_doc(".Rproj")
    if (!.rproj(file)) {uj::stopperr("The selected File is not an R project.")}
    base::gc(verbose = F)
    ...
    doc <- uj::as_path(doc)
    base::gc(verbose = F)
    if (!base::file.exists(doc)) {uj::stopperr("No such R project")}
    base::gc(verbose = F)
