...
go <- function(...) {
  ...
  name <- uj::f0(narg > 1, stop(uj::format_errs(pkg = "uj", "Invalid call")), uj::f0(narg == 1, name[2], NULL))
  if (base::is.null(name)) {
    name <- uj::choose_doc(ext)
    if (rproj(name)) {rstudioapi::openProject(name)} else {stop(uj::format_errs(pkg = "uj", "The selected file is not an R project."))}
  } else {
    ...
    doc <- uj::as_path(doc)
    base::gc(verbose = F)
    if (base::file.exists(doc)) {rstudioapi::openProject(path = doc)} else {stop(uj::format_errs(pkg = "uj", "No such R project"))}
  }
  base::gc(verbose = F)
}
...
#' @rdname go
#' @export
rebuild <- function() {devtools::document(roclets = base::c('rd', 'collate', 'namespace')); base::gc(verbose = F)}
