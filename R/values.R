...
#' @section \code{values}: Gets a named list where names are the arguments
#'   submitted to \code{v(...)}, element values are the values of the constants,
#'   and the attribute \code{'descriptions'} gives verbose descriptions if
#'   needed.
#' @section \code{vnames}: Gets a character vector of all available \code{uj}
#'   package constant names.
#' @section \code{v}: Gets one or more \code{uj} package constants by name.
