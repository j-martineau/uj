...
#'  \cr\cr `x` *with `NA` values replaced* \cr (properties may change with `NA` replacement) \cr   `sub_na`
#'  \cr\cr `x` *with `NA` values removed* \cr (properties may change with `NA` removal) \cr   `rm_na`
...
na <- function(x) {
  ...
  stop(uj::format_errs(pkg = "uj", "[x] must be populated (?ipop) and atomic, a populated atomic vlist (?ivls), or a populated atomic tabular (?itab)."))
}
...
sub_na <- function(x, s) {
  ...
  if (base::is.atomic(x)) {
    ...
    else {stop(uj::format_errs(pkg = "uj", "[x] and [s] are of incompatible modes (?compatible)."))}
    ...
    if (base::all(base::apply(x, 2, base::is.atomic))) {
      ...
      else {stop(uj::format_errs(pkg = "uj", "[s] is incompatible (?incompatible) with one or more columns of [x]."))}
    } else {stop(uj::format_errs(pkg = "uj", "When [x] is a dtf (?is_dtf), its columns must be atomic."))}
    ...
    if (base::all(base::sapply(x, base::is.atomic))) {
      ...
      else {stop(uj::format_errs(pkg = "uj", "[s] is incompatible (?incompatible) with one or more elements of [x]."))}
    } else {stop(uj::format_errs(pkg = "uj", "When [x] is a vlist (?is_vls), its elements must be atomic."))}
  } else {stop(uj::format_errs(pkg = "uj", "[x] must be an atomic vlist (?atm_vls), atomic dtf (?idtf), or some other atomic object."))}
}
...
rm_na <- function(x) {
  ...
  stop(uj::format_errs(pkg = "uj", "[x] must be populated (?ipop) and either an atomic object, an atomic vlist (?atm_vls), or an atomic dtf (?atm_dtf)."))
}
