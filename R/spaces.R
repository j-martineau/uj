...
#' @export
spaces <- function(n) {
  if (!uj::cmp_nnw_scl(n)) {stop(uj:::.errs("[n] must be a complete positive whole-number scalar (?cmp_psw_scl)."))}
...
#' @export
sqz <- function(...) {
  if (!base::all(base::sapply(base::list(...), uj::cmp_chr))) {stop(uj:::.errs("[...] must contain at least one argument, and all must be complete character vecs (?cmp_chr_vec)."))}
