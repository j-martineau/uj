...
say <- function(..., lev = NA) {
  ...
  if (dot & less) {stop(.errx("[...] must not resolve to a blank string when [lev] is in c(1, 2, 3, 4, 5)."))}
  if (!isID(msg, ""))
  if (notID(msg, "") & notID(lev, 6)) {
    if (!nas(lev)) {
      ...
      } else {stop(.errx("[lev] must be scalar NA or a positive whole-number scalar from c(1, 2, 3, 4, 5)."))}
    } else {cat(msg)}
  } else {cat(".")}
}
