...
#'                               or as an integer vector whose length is the same as the number of elements in `x` that are duplicates of earlier elements (when `int = TRUE`).    \cr   \tab  }
#' \tabular{ll}{  `dups`    \tab \link[=av]{Atomizes} `...`, then reduce the resulting vector to contain one copy of each duplicated element.                                                  }
...
#' @return **A logical vector** (when `int = FALSE)` \cr `idups`
#' \cr\cr  **An integer vector** (when `int = TRUE`) \cr `idups`
#' \cr\cr  **An atomic vector**                      \cr `dups`
#' @examples
...
idups <- function(x, int = F) {
  uj::errs_if_nots(uj::atm_vec(x), "[x] must be an atomic vec (?atm_vec).", uj::TF(int), "[int] must be TRUE or FALSE.", PKG = "uj")
