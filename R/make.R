...
#' @param cn. Optional \link[cmp_chr_vec]{complete character vec} of names to
...
#' @param r. \link[cmp_psw_scl]{Complete positive whole-number scalar} giving
#'   number of replications.
#' @param nr. \link[cmp_psw_scl]{Complete positive whole-number scalar} giving
#'   number of rows.
#' @param nc. \link[cmp_psw_scl]{Complete positive whole-number scalar} giving
#'   number of columns.
...
#' @return An \link[=idtf]{atomic dtf}, an \link[=vec]{atomic vect}, an
#'   \link[=imat]{atomic matrix}, or a \link[=is_vls]{vlist.} All others return
#'   the value of function they are thin wrappers for.
#' @export
...
vec. <- function(...) {
  ...
  x <- dw0("c(", glst(peq(x, x)), ")")                                           # create the call [list(<var1> = <var1>, <var2> = <var2>, ...)]
...
diag. <- function(x. = 1, r. = 1) {
  ...
  vr <- cmp_psw_scl(r.)
  vrs <- f0(length(x.) > 1, T, f0(vr, T, r. > 1))
  err <- NULL
