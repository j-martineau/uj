...
#'  \cr\cr
#'  Functions declaring \link[=atm_dtf]{atomic data.frames}:\tabular{rl}{
...
#' @return \tabular{rl}{
#'   `dtf,dtf.,dtf0,dtf_na`   \tab An atomic data.frame.
#'   \cr  `vec,vec.,vec_na`   \tab An atomic vector.
#'   \cr         `vls,vls.`   \tab A \link[=ivls]{vlist}.
...
dtf <- function(..., cn. = NULL) {
  ...
  if (!(ok.0 & ok.lb1 & ok.lb2 & ok.lbs)) {
    stop(c(f0(ok.0  , NULL, "\n \u2022 [...] is empty."),
           f0(ok.lb1, NULL, "\n \u2022 the number of column names in [cn.] must equal ...length() when [cn.] is not NULL."),
           f0(ok.lb2, NULL, "\n \u2022 [...] arguments must be named when [cn. = NULL]."),
           f0(ok.lbs, NULL, "\n \u2022 column names must contain only unaccented English letters, numerals, periods, and underscores. They must begin with a letter or a period followed by a letter and must end with a letter or numeral.")))
  }
  ns <- lengths(x)
  mx <- max(ns)
  ...
  errs <- c(f0(ok1, NULL, "\n \u2022 [...] arguments must be atomic vecs (?atm_vec)."),
            f0(ok2, NULL, "\n \u2022 [...] arguments may not be of length 0."        ),
            f0(ok3, NULL, "\n \u2022 [...] arguments are not recyclable."            ),
...
dtf. <- function(...) {
  ...
  errs <- c(f0(nok, NULL, "\n \u2022 [...] is empty."),
            f0(unq, NULL, "\n \u2022 [...] arguments must be uniquely named."),
            f0(avc, NULL, "\n \u2022 [...] arguments must be atomic vecs (?atm_vec)."),
            f0(rec, NULL, "\n \u2022 [...] arguments are not recyclable."),
            f0(lab, NULL, "\n \u2022 [...] argument names must contain only unaccented English letters, numerals, periods, and underscores. They must begin with a letter or a period followed by a letter and must end with a letter or numeral."))
  if (!is.null(errs)) {stop(errs)}
...
mat <- function(..., r = 1, nr = NULL, nc = NULL, br = F, rn = NULL, cn = NULL) {
  ...
  if (!ok.rn | !ok.cn) {
    stop(c(f0(ok.rn, NULL, "\n \u2022 The number of rownames in [rn] does not match the number of rows."),
           f0(ok.cn, NULL, "\n \u2022 The number of colnames in [cn] does not match the number of cols.")))
  }
  errs <- NULL
  if (!is.null(rn)) {if (!.labs_ok(rn)) {errs <- c(errs, "\n \u2022 row names must contain only unaccented English letters, numerals, periods, and underscores. They must begin with a letter or a period followed by a letter and must end with a letter or numeral.")}}
  if (!is.null(cn)) {if (!.labs_ok(cn)) {errs <- c(errs, "\n \u2022 col names must contain only unaccented English letters, numerals, periods, and underscores. They must begin with a letter or a period followed by a letter and must end with a letter or numeral.")}}
  if (!is.null(errs)) {stop(errs)}
...
vec <- function(..., r. = 1, en. = NULL) {
  ...
  errs <- c(f0(               cmp_nnw_scl(r. ), NULL, "\n \u2022 [r.] must be a non-negative whole-number scalar (?cmp_nnw_scl)."),
            f0(is.null(en.) | cmp_chr_vec(en.), NULL, "\n \u2022 [en.] must be NULL or a complete character vec (?cmp_chr_vec)." ))
  if (!is.null(errs)) {stop(errs)}
  if (r. > 1) {x <- rep.int(x, r.)}
  if (idef(en.)) {
    en. <- av(strsplit(en., "|", fixed = T))
    if (length(x) != length(en.)) {stop("\n \u2022 [en.] must be the same length as the vector resulting from atomizing (?av) [...].")}
    if (any(en. == "")          ) {stop("\n \u2022 [en.] contains a blank string (after splitting along pipes).")}
    if (!.labs_ok(en.)          ) {stop("\n \u2022 [...] argument names must contain only unaccented English letters, numerals, periods, and underscores. They must begin with a letter or a period followed by a letter and must end with a letter or numeral.")}
    names(x) <- en.
  }
  x
...
vec. <- function(...) {
  ...
  lab <- f0(!nok | !unq, T, .labs_ok(labs))
  errs <- c(f0(nok, NULL, "\n \u2022 [...] is empty."),
            f0(unq, NULL, "\n \u2022 [...] argument names must be uniquely."),
            f0(asc, NULL, "\n \u2022 [...] arguments must be atomic and scalar (?atm_scl)."),
            f0(lab, NULL, "\n \u2022 [...] argument names must contain only unaccented English letters, numerals, periods, and underscores. They must begin with a letter or a period followed by a letter and must end with a letter or numeral."))
  if (!is.null(errs)) {stop(errs)}
...
vls <- function(..., en. = NULL) {
  ...
  if (!is.null(errs)) {stop(errs)}
  if (!is.null(en.)) {if (!.labs_ok(en.)) {stop("\n \u2022 element names must contain only unaccented English letters, numerals, periods, and underscores. They must begin with a letter or a period followed by a letter and must end with a letter or numeral.")}}
