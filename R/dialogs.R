...
dirbox <- function(..., d = getwd()) {
  ...
  if (!cmp_chr_scl(d)) {err <- c(err, "\n • No default directory provided in [d].")}
  else if (!dir.exists(d)) {err <- c(err, da0("\n • [d = '", d, "'] is not an existing directory/folder."))}
  msgbox(da0("In the next dialog box, ", dot)); svDialogs::dlg_dir(default = d, title = dot)
}
...
ask <- function(..., def = "", cancel = ".stop") {
  ...
  ans <- ansbox(x = paste0(av(ends, ..., ends), collapse = ""), def = def)$res   # build the question and ask the user the question
...
ask1 <- function(opts, what, more = FALSE) {
  ...
  lst <- paste(nums, "=", opts)                                                  # list of options with preceding numbers separated by newlines
  lst <- f0(inll(suff), lst, paste0(lst, "\n\nnone = none of these options", collapse = ""))
  qst <- paste0("Enter the number of one ", what, " from these options:\n\n", collapse = "")
  qst <- paste0(qst, "\n", lst, "\n", suff)                                      # build the question separated by newlines
...
askn <- function(opts, what, all = TRUE, none = TRUE, more = FALSE) {
  ...
  val <- as_chr(1:nx(opts))                                                      # valid option numbers
  lst <- paste0(c("\n", paste0(val, "=", opts)), collapse = "\n")                # options preceded by numbers
  qst <- spf(paste0("Enter a sequence of 1 or more numbers of %s ",              # prompt
                    "from these options:\n\n"), what)                            # |
  if (all | none) {                                                              # IF all or none are valid responses
    lst <- c(lst, "\n")                                                          # : separate from numeric list by two newlines
    if (all) {
      lst <- c(lst, "all = all of the above")
      val <- c(val, "all")
    }
    if (none) {
      lst <- c(lst, "none = none of the above")
      val <- c(val, "none")
    }
    if (more) {
      lst <- c(lst, spf("done = stop selecting %s", what))
      val <- c(val, "done")
  }}                                                                             # END
  qst <- paste0(qst, paste0(lst, collapse = "\n"), f0(inll(suff), suff, c("\n\n", suff)))
  ...
  ans <- av(strsplit(ans, " ", fixed = T))
  ans <- av(strsplit(ans, ",", fixed = T))
  ans <- ans[ans != ""]                                                          # remove any blanks
  ...
  if (length(ans) == 0 & more) {return(ans)}                                     # IF no options were chosen and more are possible, return answer
  if (setequal(uv(ans), ans)) {if (allIN(ans, val)) {                            # IF all options selected are unique > IF all options are valid
...
asksn <- function(opts, what, per = 9) {
  ...
  while (!done) {                                                                # WHILE not yet done
    ...
    wht <- paste0(f0(i > 1 & ipop(out), "additional ", "blank"), what)           # : add "additional" as prefix to [whats] if this is not the first round
...
asknew <- function(old, what, unq = T) {
  ...
  ans <- av(strsplit(ans, ",", fixed = T))
  ans <-  ans[ans != ""]                                                         # remove any blanks
  if (length(ans) == length(old) & (!unq | setequal(uv(ans), ans))) {return(ans)}# IF valid THEN return
