...
inc <- function(x, i = 1, na = F) {
  ...
  errs <- c(f0(ok.x                                       , NULL, "[x] must be populated numeric object (?pop_num), a numeric dtf (?num_dtf), or a numeric vlist (?num_vls)."),
            f0(cmp_num_scl(i)                             , NULL, "[i] must be a complete numeric scalar (?cmp_num_scl)."),
            f0(isTF(na)                                   , NULL, "[na] must be TRUE or FALSE."),
            f0(f0(!ok.x | !isF(na), T, !any(is.na(av(x)))), NULL, "[x] contains NA values but [na = FALSE]."))
...
dec <- function(x, d = 1, na = F) {
  ok.x <- f0(inum(x), T, f0(num_dtf(x), T, num_vls(x)))
  errs <- c(f0(ok.x                                       , NULL, "[x] must be populated numeric object (?pop_num), a numeric dtf (?num_dtf), or a numeric vlist (?num_vls)."),
            f0(cmp_num_scl(d)                             , NULL, "[d] must be a complete numeric scalar (?cmp_num_scl)."),
            f0(isTF(na)                                   , NULL, "[na] must be TRUE or FALSE."),
            f0(f0(!ok.x | !isF(na), T, !any(is.na(av(x)))), NULL, "[x] contains NA values but [na = FALSE]."))
  if (!is.null(errs)) {stop(.errs(errs))}
  x - d
