...
case <- function(name., ..., names. = NULL, def. = "err") {
  ...
  errs <- c(f0(iscl(name.),     NULL, .errx("[name.] must be a non-NA atomic scalar (?cmp_scl).")),
            f0(...length() > 0, NULL, .errx("[...] is empty.")),
            f0(ok.names,        NULL, .errx("[names.] must be NULL or an atomic vector of length equal to ...length()")))
  ...
  err <- isEQ(def., 'err')
  if (err & !i1) {stop(.errx("[name.] does not match any argument in [...]."))}
