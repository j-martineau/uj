...
#'   \cr           \tab  
#'   \cr `tohex`   \tab A character object containing non-negative whole-number hexadecimal values.
#' }
#' @examples
#' dec. <- sample(1:100, 5)
...
todec <- function(x) {
  ...
  errs <- c(f0(ok.x   , NULL, .errx("[x] must be of mode character, have at least one value, contain no NA values, and contain only the characters in '#0123456789ABCDEFabcdef'.")),
            f0(ok.emp , NULL, .errx("After removing pound signs ('#'), one or more elements of [x] is a blank string.")),
            f0(ok.pref, NULL, .errx("If pound signs ('#') are used, they must be the first character of any element of [x] in which they are used.")))
