...
fsub <- function(x, pats, subs) {
  errs <- c(f0(cmp_chr_gen(x)              , NULL, .errx("[x] must be a complete character generic (?cmp_chr_gen).")),
            f0(cmp_chr_vec(pats)           , NULL, .errx("[pats] must be a complete character vec (?cmp_chr_vec).")),
            f0(cmp_chr_vec(subs)           , NULL, .errx("[subs] must be a complete character vec (?cmp_chr_vec).")),
            f0(recyclable(pats, subs)      , NULL, .errx("[pats] and [subs] are not recyclable.")),
            f0(length(subs) <= length(pats), NULL, .errx("There are more substitute strings than patterns to replace: length(subs) > length(pats).")))
