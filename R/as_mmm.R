...
#' @description Functions to coerce objects to \link[base:mode]{base mode} or \code{\link[=mode]{xmode}}.
...
#' @param mode A \link[mode]{three-char scalar} containing an \link[mode]{extended mode} value.
#' @param levs Optional \link[=cmp_vec]{complete atomic vec} of factor levels (ordered factor levels for `asORD`).
...
#' \cr\cr  **An object of \link[=mode]{xmode}** `'ord'` (ordered factor)     \cr\cr `as_ord`
...
as_mmm <- function(x, mode, levs = NULL, ..NA = FALSE) {
  ...
  else if (uj::not_IN(base::tolower(mode), Valid)) {uj::stopperr(Error, .PKG = "uj")}
