#' @name fx
#' @family meta
#' @title Fork:Extended Functionality for \code{\link[base]{ifelse}}
#' @description If \code{test = TRUE}, returns \code{yes}. If \code{test} is
#'   anything else, returns \code{no}.
...
#' @describeIn fx  If \code{test = TRUE}, returns \code{yes}. If \code{test =
...
#' @describeIn fx Evaluates \code{test} (which must be a logical scalar or
...
#' @name fx.
#' @title Bare-bones wrappers for \code{\link[base]{ifelse}}.
#' @description Functions wrapping \code{\link[base]{ifelse}}, expecting (but
...
#' @describeIn fx. If \code{test} is logical scalar \code{TRUE}, return
#'   \code{yes} (all of it), otherwise return \code{no} (all of it).
#' @export
f0. <- function(test, yes, no) {if (isTRUE(test)) {yes} else {no}}

#' @describeIn fx. Create a new vector the same length as \code{test}, placing
