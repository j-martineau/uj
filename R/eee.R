...
#' @export
eee_help <- function() {utils::help("eee_help", package = "uj")}

#' @describeIn eee_help Lists all effective dimensionality properties possessed by `x`. Returns a lowercase character scalar.
#' @export
eee <- function(x) {
  y <- NULL
...
#' @describeIn eee_help Lists all effective dimensionality checking functions. Returns a sorted, uppercase, character vector.
#' @export
eee_funs <- function() {base::c("E0D", "E1D", "E2D", "EHD", "EUD")}

#' @describeIn eee_help Lists all effective dimensionality properties. Returns a sorted, lowercase, character vector.
#' @export
eee_props <- function() {base::c("e0d", "e1d", "e2d", "ehd", "eud")}

#' @describeIn eee_help Checks whether `spec` is an effective dimensionality property spec. Returns a logical scalar. See \code{\link{ppp}} for a definition of a property spec.
#' @export
...
#' @describeIn eee_help Checks whether `x` matches the effective dimensionality property spec in `spec` subject to any count or value restrictions in `...`. Returns a logical scalar. See \code{\link{ppp}} for a definition of a property spec.
#' @export
EEE <- function(x, spec, ...) {
...
#' @describeIn eee_help Checks whether `x` is of zero effective dimensions subject to any count or value restrictions in `...`. Returns a logical scalar.
#' @export
E0D <- function(x, ...) {uj::EEE(x, 'e0D', ...)}

#' @describeIn eee_help Checks whether `x` is of one effective dimension subject to any count or value restrictions in `...`. Returns a logical scalar.
#' @export
E1D <- function(x, ...) {uj::EEE(x, 'e1D', ...)}

#' @describeIn eee_help Checks whether `x` is of two effective dimensions subject to any count or value restrictions in `...`. Returns a logical scalar.
#' @export
E2D <- function(x, ...) {uj::EEE(x, 'e2D', ...)}

#' @describeIn eee_help Checks whether `x` is of three or more effective dimensions subject to any count or value restrictions in `...`. Returns a logical scalar.
#' @export
EHD <- function(x, ...) {uj::EEE(x, 'eHD', ...)}

#' @describeIn eee_help Checks whether `x` is of undefined effective dimensionality subject to any count or value restrictions in `...`. Returns a logical scalar.
#' @export
EUD <- function(x, ...) {uj::EEE(x, 'eUD', ...)}

#' @describeIn eee_help Gets the effective dimensionality of `x`. Returns either `NaN` or a non-negative integer scalar.
#' @export
neee <- function(x) {
