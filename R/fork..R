...
#' @return \tabular{rl}{
#'      `f0,f1`   \tab An R object.
#'   \cr `fork`   \tab A length-`length(x)` atomic object.
#' }
#' @examples
...
fork <- function(x, y, n, na = n) {
  ...
  errs <- c(f0(ok.x , NULL, "\n \u2022 [x] must be a complete logical vec (?cmp_lgl_vec)."),
            f0(ok.y , NULL, "\n \u2022 [y] must be of length 1 or a vector of the same length as [x]."),
            f0(ok.n , NULL, "\n \u2022 [n] must be of length 1 or a vector of the same length as [x]."),
            f0(ok.na, NULL, "\n \u2022 [na] must be of length 1 or a vector of the same length as [x]."))
  ...
  errs  <- c(f0(ok.arg, NULL,            "\n \u2022 [na = 'err'] but [x] contains NA values."),
             f0(ok.tny, NULL, f0(inc.na, "\n \u2022 [y], [n], and [na] must be of compatible (?compatible) modes.",
     ...
     f0(isFALSE(x), n,
        f0(nas & !isID(na, 'err'), na,
           f0(!isLG(x) & !isID(err, 'err'), err,
              f0(nas, stop("\n \u2022 [x] must be atomic, scalar, and TRUE, FALSE, or NA."),
                      stop("\n \u2022 [x] must be atomic, scalar, and TRUE or FALSE." ))))))
}
