...
#' ccc(letters)
#' @export
ccc_PROPS <- function() {utils::help("extended_class_properties", package = "uj")}
...
ccc <- function(x) {
  ...
  for (CCC in uj::ccc_funs()) {
    match <- base::eval(base::parse(text = base::paste0("uj::.", CCC, "(x)")))
...
CCC <- function(x, spec, ...) {
  ...
  if (uj::meets(x, ...)) {
    ...
    for (prop in props) {if (base::eval(base::parse(text = base::paste0("uj::.", prop, "(x)")))) {return(T)}}
  }
  F
}
