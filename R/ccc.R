...
#' @param x An object.
#' @param xxx \code{NULL} or \link[cmp_chr_scl]{complete character scalar}
...
atm_dtf <- function(x, xxx = NULL) {
  vxxx <- ifelse(is.null(xxx), T, isIN(xxx, mmm_vals()))
  if (!vxxx) {stop("\n • [xxx] must be NULL or a character scalar value from mmm_vals().")}
  if (!pop_dtf(x)) {F} else if (!all(apply(x, 2, is.atomic))) {F} else if (is.null(xxx)) {T}
  else {
...
#' @export
ccc <- function(x) {
  c(if (atm_arr(x)) {'arr'} else {NULL}, if (atm_gen(x)) {'agn'} else {NULL}, if (atm_dtf(x)) {'atb'} else {NULL},
    if (atm_vls(x)) {'avl'} else {NULL}, if (atm_vtp(x)) {'avt'} else {NULL}, if (atm_mat(x)) {'mat'} else {NULL},
    if (atm_mvc(x)) {'mvc'} else {NULL}, if (atm_scl(x)) {'scl'} else {NULL}, if (atm_vec(x)) {'vec'} else {NULL})
}
