...
swap <- function(x, from = NULL, to = NULL, old = NULL, new = NULL, .dim = 0, .all = FALSE) {
  errs <- NULL
  if (!ppp::.pop_dtf(x) & !ppp::.pop_mat(x) & !ppp::.pop_atm(x) & !ppp::.pop_vls(x)) {errs <- base::c(errs, "[x] must be a populated matrix, data.frame, vlist, or an atomic object (?VLS).")}
  if (ppp::cmp_nnw_scl(.dim)) {if (base::any(!(.dim %in% 0:2))) {errs <- base::c(errs, "[.dim] must be 0, 1, or 2.")}}
  ...
  if (ppp::.dTF(x) | uj::.MAT(x)) {
    if (.dim == 0) {errs <- base::c(errs, "[.dim] must be 2 or 2 when [x] is a data frame or matrix.")}
    if (!ppp::NLL(old)) {errs <- base::c(errs, "[old] must be NULL when [x] is a data frame or matrix.")}
    if (!ppp::NLL(new)) {errs <- base::c(errs, "[new] must be NULL when [x] is a data frame or matrix.")}
    if (!ppp::unq_whl_vec(from)) {errs <- base::c(errs, "[from] must be a uniquely valued whole-number vector.")}
    if (!ppp::unq_whl_vec(from)) {errs <- base::c(errs, "[to] must be a uniquely valued whole-number vector.")}
    if (ppp::DEF(errs)) {ppp::stopperr(errs, pkg = "uj")}
  ...
  } else if (ppp::DEF(old) | ppp::DEF(new)) {
    if (!ppp::.pop_atm(x)) {errs <- base::c(errs, "[old] and/or [new] are supplied, but [x] is not atomic.")}
    if (.dim != 0) {errs <- base::c(errs, "[.dim] must be 0 when [x] is a vector or vlist (?uj::VLS).")}
    if (!ppp::unq_atm_vec(old)) {errs <- base::c(errs, "[old] must be a unique-valued atomic vector (?ppp::unq_atm_vec).")}
    if (!ppp::atm_vec(new)) {errs <- base::c(errs, "[new] must be an atomic vector (?ppp::atm_vec).")}
    if (ppp::DEF(errs)) {ppp::stopperr(errs, pkg = "uj")}
    ...
    if (!ppp::DEF(from)) {errs <- base::c("When [x] is a vector or vlist (?uj::VLS) and neither [old] nor [new] is supplied, both [from] and [to] must be supplied.")}
