...
#' egXb <- egXb()
#' @export
xb_funs <- function() {utils::help("xb_funs", package = "uj")}

#' @describeIn xb_funs Columns binds any compatible non-`NULL` `...` arguments. If there is only one non`NULL` argument and it is a matrix or data.frame, returns that argument.
#' @export
cb <- function(...) {
  if (base::...length() > 0) {
    x <- base::list(...)
...
#' @describeIn xb_funs Row binds any compatible non-`NULL` `...` arguments. If there is only one non`NULL` argument and it is a matrix or data.frame, returns that argument.
#' @export
rb <- function(...) {
  if (base::...length() > 0) {
    x <- base::list(...)
