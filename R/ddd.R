...
#' ddd(letters)
#' @export
ddd_PROPS <- function() {utils::help("ddd_PROPS", package = "uj")}

#' @describeIn ddd_PROPS Lists all defined dimensionality properties possessed by `x`. Returns a lowercase character scalar.
#' @export
ddd <- function(x) {
  Y <- NULL
  for (ddd in uj::ddd_funs()) {if (base::eval(base::parse(text = base::paste0("uj::.", ddd, "(x)")))) {Y <- base::c(Y, base::tolower(DDD))}}
  Y
}
