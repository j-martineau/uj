...
#'   \cr             \tab  
#'   \cr `anys(.)`   \tab Evaluates each sweep for any `TRUE` values.
#'   \cr             \tab  
#'   \cr `alls(.)`   \tab Evaluates each sweep for only `TRUE` values.
#'   \cr             \tab  
#'   \cr `ones(.)`   \tab Evaluates each sweep for exactly `1` `TRUE` value.
#'   \cr             \tab  
#'   \cr `twos(.)`   \tab Evaluates each sweep for `2+` `TRUE` values.
#' }
...
not <- function(x, na = 'err') {
  errs <- c(f0(pop_lgl(x)                , NULL, .errx("[x] must be a populated logical object (?cmp_lgl).")),
            f0(isLG(na) | isEQ(na, 'err'), NULL, .errx("[na] must be TRUE, FALSE, NA, or 'err'.")))
...
and <- function(x, y, na = 'err') {
  errs <- c(f0(pop_lgl(x)                , NULL, .errx("[x] must be a populated logical object (?cmp_lgl).")),
            f0(pop_lgl(y)                , NULL, .errx("[x] must be a populated logical object (?cmp_lgl).")),
            f0(.conform(x, y)            , NULL, .errx("[x] and y must be conformable.")),
            f0(isLG(na) | isEQ(na, 'err'), NULL, .errx("[na] must be TRUE, FALSE, NA, or 'err'.")))
  if (!is.null(errs)) {stop(errs)}
...
TEST <- function(x, na = FALSE, err = NA) {
  errs <- c(f0(isTF(na)                    , NULL, .errx("[na] must be TRUE or FALSE.")),
            f0(isLG(err) | isEQ(err, 'err'), NULL, .errx("[err] must be TRUE, FALSE, NA, or 'err'.")))
  if (!is.null(errs)) {stop(errs)}
  if (!lgl_scl(x) & !isLG(err)) {stop(.errx("[x] is not a logical scalar."))} else if (!lgl_scl(x)) {err} else if (is.na(x)) {na} else {x}
}

#' @rdname evals
#' @export
ANY <- function(..., err = NA) {
  errs <- c(f0(all(sapply(list(...), lgl_scl)), NULL, .errx("All arguments in [...] must be logical scalars (?lgl_scl).")),
            f0(isLG(err) | isEQ(err, 'na')    , NULL, .errx("[err] must be TRUE, FALSE, NA, or 'na'.")))
  if (!is.null(errs)) {stop(errs)}
  for (i in 1:...length()) {
    ...
    if (!vx) {
      if (isTF(err)) {x <- err}
      else {stop(.errx("[..", i, "] did not resolve to TRUE or FALSE."))}
    }
    if (x) {return(T)}
  }
  F
...
ALL <- function(..., err = NA) {
  ...
  for (i in 1:...length()) {
    x <- tryCatch(...elt(i) == T, error = function(e) NA, finally = NULL)
    vx <- cmp_lgl_scl(x)
    if (!vx) {if (isTF(err)) {x <- err} else {stop(.errx("[..", i, "] did not resolve to TRUE or FALSE."))}}
    if (!x) {return(F)}
  }
  T
}
...
#' @export
TWO <- function(..., err = NA) {
  errs <- c(f0(all(sapply(list(...), lgl_scl)), NULL, .errx("All arguments in [...] must be logical scalar (?lgl_scl).")),
            f0(isLG(err) | isEQ(err, 'na')    , NULL, .errx("[err] must be TRUE, FALSE, NA, or 'na'.")))
  if (!is.null(errs)) {stop(errs)}
  n <- 0
  for (i in 1:...length()) {
...
tests <- function(..., a = FALSE, na = FALSE, not = FALSE, agg = NA, sweep = NA) {
  ...
  errs <- c(f0(isTF(a)  , NULL, .errx("[a] must be TRUE or FALSE.")),
            f0(isTF(na) , NULL, .errx("[na] must be TRUE or FALSE.")),
            f0(isTF(not), NULL, .errx("[not] must be TRUE or FALSE.")),
            f0(ok.r     , NULL, .errx("[...] arguments are not recyclable (?recyclable).")),
            f0(ok.s     , NULL, .errx("[sweep] must be NA, 'across', 'within', or 'both'.")),
            f0(ok.a     , NULL, .errx("[agg] must be NA, 'nor', 'one', 'any', 'two', 'all'.")),
            f0(ok.sa    , NULL, .errx("[agg] and [sweep] must both be NA or must both be non-NA.")))
  if (!is.null(errs)) {stop(errs)}
  for (i in 1:length(x)) {
    arg <- x[[i]]
    arg[is.na(arg)] <- na
...
nors <- function(..., a = FALSE, na = FALSE, sweep = "across") {
  ...
  else {stop(.errx("[sweep] must be 'across', 'within', or 'both'."))}
}
...
isin <- function(x, y, na = FALSE, agg = NA, not = FALSE) {
  ...
  errs <- c(f0(ok.x     , NULL, .errx("[x] must be a populated atomic vec (?pop_vec).")),
            f0(ok.y     , NULL, .errx("[y] must be a populated atomic vec (?pop_vec).")),
            f0(isTF(na) , NULL, .errx("[na] must be TRUE or FALSE.")),
            f0(ok.agg   , NULL, .errx("[agg] must be NA, 'nor', 'one', 'any', 'two', or 'all'.")),
            f0(isTF(not), NULL, .errx("[not] must be TRUE or FALSE.")),
            f0(ok.xy    , NULL, .errx("[x] and [y] are not of compatible modes.")))
  if (!is.null(errs)) {stop(errs)}
  out <- x %in% y
  out[x[is.na(x)]] <- na
  n <- length(which(out))
