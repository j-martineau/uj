...
#' prop_defs()
#' @export
ppp_prop_funs <- function() {utils::help("ppp_prop_funs", package = "uj")}

#' @describeIn ppp_prop_funs Lists all properties of `x`. Returns a sorted, lowercase, character vector.
#' @export
ppp <- function(x) {
  base::unique(base::sort(base::c(uj::bbb(x), uj::ccc(x), uj::ddd(x), uj::eee(x), uj::iii(x), uj::mmm(x), uj::sss(x))))
}

#' @describeIn ppp_prop_funs Lists all property-or functions. Returns a sorted, lowercase, two-element, character vector.
#' @export
ppp_or_funs <- function() {base::c("na0_or", "nll_or")}

#' @describeIn ppp_prop_funs Lists all possible properties. Returns a sorted, lowercase, character vector.
#' @export
...
#' @describeIn ppp_prop_funs Lists all possible property checking functions. Includes both single-property and combination property checking functions. Returns a sorted, lowercase, character vector.
#' @export
...
#' @describeIn ppp_prop_funs Checks whether `x` is the name of a property checking function. Returns a logical scalar.
#' @export
...
#' @describeIn ppp_prop_funs Checks whether `x` is a single property. Returns a logical scalar.
#' @export
is_prop <- function(x) {if (uj::.cmp_ch3_scl(x)) {base::tolower(x) %in% uj::all_props()} else {F}}

#' @describeIn ppp_prop_funs Checks whether `x` is a combination property spec. Returns a logical scalar. A combination property spec is two or more unique, valid, single properties separated by underscores.
#' @export
...
#' @describeIn ppp_prop_funs Converts a property spec to a unique list of its constituent single properties by splitting `x` along pipes and underscores, removing any blank values, sorting the remaining values, and returning the unique set of remaining values.
#' @export
...
#' @describeIn ppp_prop_funs Converts a property spec into a vector of combination properties. Returns a character vector.
#' @export
combos_from_spec <- function(x) {base::sort(base::unique(uj::spec2combos(x)))}

#' @describeIn ppp_prop_funs Converts a combined property spec into a vector of single properties. Returns a character vector.
#' @export
...
#' @describeIn ppp_prop_funs Checks `x` against the property spec `spec` subject to any count or value restrictions in `...`. Returns a logical scalar.
#' @export
PPP <- function(x, spec, ...) {
...
#' @describeIn ppp_prop_funs Checks `x` against null-ness or the property spec `spec` subject to any count or value restrictions in `...`. Returns a logical scalar.
#' @export
nll_or <- function(x, spec, ...) {if (base::is.null(x)) {T} else {uj::PPP(x, spec, ...)}}

#' @describeIn ppp_prop_funs Checks `x` against scalar missingness-ness or the property spec `spec` subject to any count or value restrictions in `...`. Returns a logical scalar.
#' @export
na0_or <- function(x, spec, ...) {if (uj::.NA0(x)) {T} else {uj::PPP(x, spec, ...)}}

#' @describeIn ppp_prop_funs Produces a data frame containing all properties with columns indicating property family, the value of the property, a short description of the property, and a long description of the property. Returns a tibble/data frame.
#' @export
prop_defs <- function() {
  tibble::tribble(
...
#' @describeIn ppp_prop_funs Converts the property spec `x` into a verbose description of each constituent (combo) property, identifying each constituent (combo) property as an alternative to the others. Returns a character scalar.
#' @export
...
#' @describeIn ppp_prop_funs Converts the (combo) property `x` to a concise description.
#' @export
...
#' @describeIn ppp_prop_funs Converts the property spec `x` to a concise description.
#' @export
