...
#' @return *A non-negative whole number object* \cr   `todec`
#'  \cr\cr *A character object of non-negative whole number hexadecimal values* \cr   `tohex`
#' @examples
#' dec. <- sample(1:100, 5)
...
todec <- function(x) {
  ...
  if (uj::cmp_chr(x)) {
    ...
    if (ok.x) {
      ...
      ok.pref <- !base::any(pp > 1)
      x  <- base::gsub("#", "", x, fixed = T)
      ok.emp <- !base::any(base::nchar(x) == 0)
  }}
  errs <- base::c(uj::f0(ok.x   , NULL, "[x] must be of mode character, have at least one value, contain no NA values, and contain only the characters in '#0123456789ABCDEFabcdef'."),
                  uj::f0(ok.emp , NULL, "After removing pound signs ('#'), one or more elements of [x] is a blank string."),
                  uj::f0(ok.pref, NULL, "If pound signs ('#') are used, they must be the first character of any element of [x] in which they are used."))
  ...
  conv <- function(xx) {                                                         # sub-function to convert a single hex value to decimal.
    ...
    nn <- base::length(xx)                                                             # number of hex digits
    ...
    xx[xx %in% c("e", "E")] <- "14"
    xx[xx %in% c("f", "F")] <- "15"
    xx <- base::as.integer(xx)                                                         # convert digits to integer
...
#' @export
tohex <- function(x) {
  if (!uj::cmp_nnw_vec(x)) {stop(uj::format_errs(pkg = "uj", "[x] must be complete, non-negative whole-number vec (?cmp_nnw_vec)."))}
