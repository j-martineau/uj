...
#' @export
wraps_data_table <- NULL

#' @describeIn wraps_data_table Coerce to data.table.
#' @inherit data.table::as.data.table
#' @export
dtas <- function(x, say = TRUE) {if (say) {say("\n| coerce to data.table")}; data.table::as.data.table(x)}

#' @describeIn wraps_data_table Evaluate whether an object is a data.table
#' @inherit data.table::is.data.table
#' @export
dtis <- function(x) {data.table::is.data.table(x)}

#' @describeIn wraps_data_table If an object is not already a data.table, coerce it.
#' @export
dtie <- function(x, say = TRUE) {if (data.table::is.data.table(x)) {x} else {dtas(x, say)}}

#' @describeIn wraps_data_table Extract a subset of a data.table.
#' @export
dtsub <- function(x, ir, ic) {x[ir , ..ic]}

#' @describeIn wraps_data_table Extract columns of a data.table.
#' @export
dtcols <- function(x, ic) {x[ , ..ic]}

#' @describeIn wraps_data_table Merge two data.tables by specified columns.
...
#' @describeIn wraps_data_table Extract rows of a data.table.
#' @export
dtrows <- function(x, ir) {x[ir, ]}

#' @describeIn wraps_data_table Reshape a data.table from long to wide.
