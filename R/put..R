...
#'       `putc`   \tab Puts the vector `v` into the `c`-indexed column of `x` (assumed to be a data.frame or matrix).
#'   \cr          \tab  
#'   \cr `putr`   \tab Puts the vector `v` into the `r`-indexed row of `x` (assumed to be a data.frame or matrix).
#'   \cr          \tab  
#'   \cr  `put`   \tab Puts values of `v` into `e`-indexed elements of `x` (assumed to be a vector or \link[=ccc]{vlist}).
#' }
