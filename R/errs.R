...
#' \tabular{rl}{
#'       `err_check`   \tab *Process any banked error messages*.
#'   \cr               \tab Checks for banked error messages in the environment of the function `gens` generations back in the call stack, and if there are any, processes them, stopping execution. If there are none, takes no action. To be called after error checking is complete.
#'   \cr               \tab   
#'   \cr `bank_funs`   \tab *Bank errors for args not meeting property functions*.
...
#'   \cr               \tab   
#'   \cr `bank_vals`   \tab *Bank errors for args with out-of-bounds values*.
...
#'   \cr               \tab   
#'   \cr `bank_dots`   \tab *Bank errors for `...` args*.
#'   \cr               \tab Bank errors if `...` arguments do not satisfy a property specification, and optionally, if they are not named. Checks if each `...` argument satisfies the property specification in `prop.` and if not, banks an error message. If `named. = TRUE`, checks whether all `...` arguments are named, and if not banks an error message.
#'   \cr               \tab   
#'   \cr `bank_when`   \tab *Bank errors for one arg conditional on the value of another*.
#'   \cr               \tab  Bank errors conditionally on the value of two arguments. Banks an error if the first named atomic scalar `...` argument has a value contained in `whens.` and the second named atomic scalar `...` argument does not contain a value from `values.`.
#'   \cr               \tab   
#'   \cr `bank_pats`   \tab *Bank errors for args not matching any patterns of properties*.
#'   \cr               \tab  Bank errors conditionally on the pattern of properties of arguments in `...` matching none of the specified patterns of associated properties in `pats.`.
#'   \cr               \tab   
#'   \cr `bank_fail`   \tab *Bank errors for args producing an error when evaluated*.
#'   \cr               \tab   
#'   \cr  `bank_err`   \tab *Bank an arbitrary error message*.
#'   \cr               \tab  Banks the error message in `...` in the environment of the function `gens` generations back in the call stack. The error message is constructed by \link[=av]{atomizing} and collapsing `...` into a character scalar.
#'   \cr               \tab   
#'   \cr  `bank_lgl`   \tab *Check for logical scalar args*.
#'   \cr               \tab Checks named `...` arguments for whether they are scalar `TRUE` or scalar `FALSE`. If `na = TRUE`, also allows atomic scalar `NA`. If `extras` contains atomic values (logical or not), also allows those values. For each named argument that does not meet the requirements, banks an error.
#'   \cr               \tab   
#'   \cr  `bank_not`   \tab *Bank errors for `FALSE` args*.
...
err_check <- function(gens. = 0) {
  ...
  if (!ok.gens) {stop(.errs("[gens.] doesn't point to a function in the call stack."))}
  gens. <- gens. + 1
...
bank_err <- function(..., gens. = 0) {
  ...
  if (!ok.err) {stop(.errs("[...] has no atomic values"))}
...
bank_lgl <- function(..., nas. = F, extras. = NULL) {
  ...
  errs <- c(f0(ok.0     , NULL, "[...] is empty."),
            f0(ok.labs  , NULL, "All arguments in [...] must be named uniquely without using blank strings."),
            f0(ok.lgl   , NULL, "All arguments in [...] must be complete logical scalars (?cmp_lgl_scl)."),
            f0(ok.nas   , NULL, "[nas.] must be TRUE or FALSE."),
            f0(ok.extras, NULL, "[extras.] must be NULL or complete and atomic (?icmp)."))
  ...
  for (i in 1:nd) {
    dot <- ...elt(i)
    ok.dot <- isTF(dot)
    if (!ok.dot & nas.) {ok.dot <- isNAS(dot)}
...
bank_not <- function(...) {
  ...
  errs <- NULL
  if (!v0 ) {errs <- c(errs, "[...] is empty.")}
  if (!vnn) {errs <- c(errs, "At least one argument in [...] must be named.")}
  if (!vnb) {errs <- c(errs, "At least one argument in [...] must be unnamed.")}
  if (!vnm) {errs <- c(errs, "Named arguments in [...] must be uniquely named (without using a blank string as a name).")}
  if (!vlg) {errs <- c(errs, "Named arguments in [...] must be complete logical scalars (?cmp_lgl_scl).")}
  if (!ver) {errs <- c(errs, "An unnamed argument in [...] must contain the escape sequence '{@}' for inserting the names of named arguments.")}
  if (idef(errs)) {stop(.errs(errs))}
...
bank_pop <- function(...) {
  ...
  errs <- c(f0(ok.n      , NULL, "[...] is empty."),
            f0(ok.unnamed, NULL, "All arguments in [...] must be named."),
            f0(ok.named  , NULL, "Names of arguments in [...] must be unique without using blank strings."))
  if (!is.null(errs)) {stop(.errs(errs))}
...
bank_funs <- function(funs., ...) {
  ...
  errs <- c(f0(ok.n   , NULL, "[...] is empty."),
            ...
            f0(ok.funs, NULL, "[funs.] contains a function name not found in ppp_funs()."))
  if (!is.null(errs)) {stop(.errs(errs))}
...
bank_vals <- function(...) {
  ...
  errs <- c(f0(ok.0   , NULL, "[...] is empty."),
            f0(ok.args, NULL, "At least one argument in [...] must be named."),
            f0(ok.vals, NULL, "At least one argument in [...] must be unnamed."),
            f0(ok.labs, NULL, "Named arguments in [...] must be uniquely named without using blank strings."),
            f0(ok.atm , NULL, "Named arguments in [...] must be non-empty and atomic."),
            f0(ok.pop , NULL, "Unnamed arguments in [...] must be non-empty and atomic."))
  if (!is.null(errs)) {stop(.errs(errs))}
  values <- NULL
  for (val in vals) {
...
bank_when <- function(whens., values., ...) {
  ...
  errs <- c(f0(ok.whens  , NULL, "[whens.] must be non-empty and atomic."),
            ...
            f0(ok.n      , NULL, "There must be two arguments in [...]"),
            f0(ok.labs   , NULL, "Arguments in [...] must be uniquely named without using blank strings."),
            f0(ok.whens2 , NULL, "[whens.] and [..1] are not of compatible (?compatible) modes."),
            f0(ok.values2, NULL, "[values.] and [..2] are not of compatible (?compatible) modes."),
            f0(ok.dots   , NULL, "Both arguments in [...] must be atomic and scalar (?iscl)."))
  if (!is.null(errs)) {stop(.errs(errs))}
  labs1 <- paste0("[", labs[1], "]")
  labs2 <- paste0("[", labs[2], "]")
  when <- ..1
